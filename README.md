...
# Azure AI Language (entities/sentiment/key phrases)
AZURE_LANGUAGE_ENDPOINT=
AZURE_LANGUAGE_KEY=
AZURE_LANGUAGE_REGION=
COSMOS_ENRICHMENTS_TTL_SECONDS=1209600
...
# Application Insights (API telemetry)
APPLICATIONINSIGHTS_CONNECTION_STRING=
APPINSIGHTS_ROLE_NAME=bharat-setu-api
APPINSIGHTS_SAMPLING_PERCENTAGE=30
APPINSIGHTS_AUTOCOLLECT_REQUESTS=false
APPINSIGHTS_AUTOCOLLECT_DEPENDENCIES=false

# RAG confidence fallback threshold
RAG_CONFIDENCE_THRESHOLD=0.62

# Storage + async pipeline
AZURE_STORAGE_CONNECTION_STRING=
AZURE_BLOB_UPLOAD_CONTAINER=bharat-setu-uploads
ASYNC_QUEUE_CLUSTER_NAME=bs-cluster-jobs
ASYNC_QUEUE_NOTIFY_NAME=bs-notify-jobs
ASYNC_QUEUE_POSTPROCESS_NAME=bs-postprocess-jobs
ASYNC_QUEUE_SCAN_NAME=bs-scan-jobs
COSMOS_UPLOADS_TTL_SECONDS=2592000
COSMOS_ASYNC_JOBS_TTL_SECONDS=1209600
...
### Demo cost-control (Azure CLI)

Use these commands once App Insights is created to keep demo telemetry cost near-zero.
...
- `--daily-cap 0.2` means cap at ~200 MB/day ingest.
- Keep app sampling at `APPINSIGHTS_SAMPLING_PERCENTAGE=30` for demo traffic.

Optional starter alerts (minimal set):

```bash
az monitor metrics alert create \
  --name bs-api-server-errors \
  --resource-group <resource-group> \
...
az monitor metrics alert create \
  --name bs-api-latency-p95 \
  --resource-group <resource-group> \
  --scopes <appinsights-resource-id> \
  --condition "percentile requests/duration 95 > 2000" \
  --window-size 10m \
  --evaluation-frequency 5m
```
...
| `/api/schemes` | POST | Scheme matching + grounded RAG answer with citations/confidence |
| `/api/uploads/sas` | POST | Issue short-lived Blob SAS upload URL + track metadata |
| `/api/uploads/finalize` | POST | Finalize upload and enqueue scan/classification job |
