...
export default function ImpactDashboard({ onClose }: { onClose: () => void }) {
  ...
  async function handleListenNews(news: typeof MOCK_NEWS[0]) {
    ...
    try {
      ...
      const base64Audio = await generateSpeechBase64(explanation, userProfile.language || 'en-IN');

      const audio = new Audio(base64Audio);
      audioRef.current = audio;

      audio.onended = () => setPlayingNewsId(null);
      await audio.play();
  ...
  async function handleListenToSummary() {
    ...
    try {
      ...
      const base64Audio = await generateSpeechBase64(fullText, userProfile?.language || 'en-IN');
      const audio = new Audio(base64Audio);
      audioRef.current = audio;
