...
import { generateSpeechBase64, generateAudioSummary } from '@/app/actions/azure-ai';
...
export default function HelpNeighbourOverlay({
  ...
  const handlePlayAudio = async () => {
    ...
    try {
      ...
      // Step 1: Generate a concise 50-word summary for audio only
      const audioSummary = await generateAudioSummary(summary, shortLang);
      
      // Step 2: Convert the short summary to speech
      const audioStr = await generateSpeechBase64(audioSummary, shortLang);
    ...
    <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-[#0a1628] flex flex-col max-w-[430px] mx-auto overflow-hidden" style={{ animation: 'slideUp 0.3s ease-out' }}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24 p-5 flex flex-col gap-6 relative">
          ...
          {summary && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 mt-2">
                 ...
                 <button 
                   onClick={() => {
                     setPreviewUrl(null); setSelectedFile(null); setSummary(null); setOcrText(null); setAnalysis(null); setAudioUrl(null);
                     if (playing) { audioRef.current?.pause(); setPlaying(false); }
                   }}
        ...
        <audio 
          ...
          onError={() => { setPlaying(false); console.error('Audio playback failed'); }}
        />
