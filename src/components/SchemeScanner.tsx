...
import { generateSpeechBase64 } from '@/app/actions/azure-ai';
...
export default function SchemeScanner({ onClose }: { onClose: () => void }) {
  ...
  const handleListenScheme = async (e: React.MouseEvent, scheme: SchemeItem) => {
    e.stopPropagation();
    ...
    try {
      ...
      const audioStr = await generateSpeechBase64(generatedScript, shortLang);
      setCurrentAudioUrl(audioStr);
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            {filteredSchemes.map((scheme, i) => (
              <button
                key={i}
                onClick={() => {
                  const newScheme = selectedScheme?.scheme_name === scheme.scheme_name ? null : scheme;
                ...
                {selectedScheme?.scheme_name === scheme.scheme_name && (
                  <div className="mt-3 pt-3 border-t border-black/10 dark:border-white/10 space-y-3" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-between">
                       <p className="text-[10px] font-bold text-slate-500 dark:text-gray-400 uppercase">{t('Eligibility')}</p>
                       <button 
                         type="button"
                         ...
                         className={`flex items-center gap-1.5 px-3 py-1 rounded-lg font-bold text-xs transition-colors ${
                           playingAudio ? 'bg-red-500/10 text-red-600' : 'bg-[#FF9933]/10 text-[#FF9933]'
                         }`}
                       >
                         {generatingAudio ? (
            ...
            <audio 
              ...
              onError={() => { setPlayingAudio(false); console.error('Audio playback failed'); }}
            />
          </div>
        )}
      </div>
