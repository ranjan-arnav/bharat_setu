...
import { translateText, generateSpeechBase64 } from '@/app/actions/azure-ai';
...
export default function BureaucracyXRay({ onClose }: { onClose?: () => void }) {
  ...
  async function handlePlayAudio(text: string) {
    ...
    try {
      const base64Audio = await generateSpeechBase64(text, lang);
      const audio = new Audio(base64Audio);
      audioRef.current = audio;

      audio.onended = () => setIsPlaying(false);
      audio.onerror = () => {
        console.warn('Network TTS failed, falling back to OS SpeechSynthesis');
        fallbackOsTTS(text);
      };
      
      await audio.play();
    } catch (e) {
      console.warn('Audio play error, falling back to OS SpeechSynthesis', e);
      fallbackOsTTS(text);
    }
  }

  function fallbackOsTTS(text: string) {
              ...
              {translatedAppId === activeId ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mt-2 bg-black/5 dark:bg-white/5 p-3 rounded-xl border border-black/10 dark:border-white/10">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <button 
                      onClick={() => handlePlayAudio(aiSummary || '')}
