...
async function deleteUserPartitionItems(containerName:
  ...
  if (!container) {
    throw new BackendHttpError(503, `Cosmos backend is not configured for ${containerName}`);
  }
...
async function deleteMessagesForUser(userId: string) {
  ...
  if (!container) {
    throw new BackendHttpError(503, 'Cosmos backend is not configured for messages');
  }
...
async function deleteSosEventsBySessionIds(sessionIds: string[]) {
  ...
  if (!eventsContainer) {
    throw new BackendHttpError(503, 'Cosmos backend is not configured for sosEvents');
  }
...
export async function resetSessionCloudData(body: Record<string, unknown>) {
  ...
  if (!sosContainer) {
    throw new BackendHttpError(503, 'Cosmos backend is not configured for sosSessions');
  }
  const sosSessionQuery = await sosContainer.items
    ...
    .fetchAll();
  const sessionIds = sosSessionQuery.resources
